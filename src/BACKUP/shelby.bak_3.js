(function(factory) {
	...
	(function() {
		...
		Shelby.Extenders.subscribe.fn.prototype = {
			subscribe: function(callback, options) {
				...
				var subscription = {
					...
					// Array having the subscriptions data for each properties of the subscriptions.
				...
				this._addToSubscription(this._target(), subscription, canSubscribeToProperty, options, {});
				...
				if (subscription.propertySubscriptions.length > 0) {
			...
			_addToSubscription: function(target, subscription, propertyEvaluator, options, context) {
					...
					if (propertyEvaluator(path)) {
						...
						var proxyCallback = function(value) {
							...
							if (utils.isObject(value)) {
								...
								if (options.array.trackChildren !== false) {
									if (value.action === "added") {
										...
										$.each(value.value, function() {
											that._addToSubscription(this, subscription, propertyEvaluator, options, {
												path: path + (utils.stringEndsWith(path, "/") ? "i" : "/i"),
										...
										$.each(value.value, function() {
											that._removeFromSubscription(this, subscription);
										});
									}
								}
							}
						...
						if (!utils.isNull(parent) && utils.hasProperty(parent, namespace)) {
							// Mark the subscription data with the parent subscriber id to ensure that the property subscription
							// is removed if the parent is removed from the subscription.
							propertySubscription.subscriberId = parent[namespace][subscriber_id_property];
						}
						
						// Save the subscription data informations for later.
						subscription.propertySubscriptions.push(propertySubscription);
					}
				};
				
				var assignSubscriberId = function(property) {
					if (!utils.hasProperty(property.value[namespace], subscriber_id_property)) {
						// Assign a unique subscriber id to the object. This unique identifier is used to retrieve and dispose the subscription
						// data of all the children of the object if he is removed from the subscription.
						property.value[namespace][subscriber_id_property] = utils.generateGuid();
					}
				};
				...
				factory.parser().parse(target, {
					filter: function(key, value, path) {
						// A property can be subscribe to if it is an observable. Object are allowed but will never be subscribed to. We cannot
					...
					onObject: assignSubscriberId,
					onArray: function(property) {
						/* VALIDER QUE ÇA FONCTIONNE AVEC LES ARRAYS */
						assignSubscriberId(property);
						subscribe(property);
					},
					onFunction: subscribe
				});
			},
			
			_removeFromSubscription: function(target, subscription) {
				var id = target[namespace][subscriber_id_property];
					
				if (!utils.isNull(id)) {
					var subscriptions = subscription.propertySubscriptions;
				
					for (var i = 0; i < subscriptions.length; i += 1) {
						if (subscriptions[i].subscriberId === id) {
							console.log("Subscription disposed");
						
							// Dispose the KO subscription.
							subscriptions[i].dispose();
						
							// Remove the subscription from the repository.
							utils.arrayRemoveIndex(subscriptions, i);
							
							// Because we removed the subscription, the array has been re-indexed
							// so we must decrement the index otherwise we would skip an item.
							i -= 1;
						}
					}
				}
		...
		var subscriber_id_property = Shelby.Extenders.subscribe.SUBSCRIBER_ID_PROPERTY = "__subscriber_id__";
		...
		Shelby.Extenders.edit.fn.prototype = {
			...
			/**** SERAIT MIEU D'APPELER ÇA hasMutated ****/
			isEdited: function() {
				var ret = false;
				...
				pausableSubscription: function(callback, callbackTarget /*, [event] */) {
					...
					var proxyCallback = function(value) {
						...
						if (!pauseAllSubscriptions && !pausableSubscription.isPause) {
							// If this observable is not paused globally and for this subscription, call the original
							// callback with the original arguments.
							callback.apply(this, [value, pausableSubscription]);
						}
					};
				...
				pausableArraySubscription: function(callback /*, [callbackTarget], [event] */) {
					...
					if (arguments[2] !== "beforeChange") {
						...
						target.subscribe(function(value) {
							previous = utils.cloneArray(value);
						}, null, "beforeChange");
					
						// Proxy callback function adding the array changes behavior.
						arguments[0] = function(value, subscription) {
							var differences = ko.utils.compareArrays(previous, value),
								added = [],
								removed = [];
							...
							if (added.length > 0) {
								callback.apply(this, [{ value: added, action: "added" }, subscription]);
							}
							
							if (removed.length > 0) {
								callback.apply(this, [{ value: removed, action: "removed" }, subscription]);
							}
						};
					}
	...
	(function() {
		...
		Shelby.ViewModel.prototype = {
			...
			dispose: function() {
				var namespace = Shelby.PropertyExtender.NAMESPACE;
